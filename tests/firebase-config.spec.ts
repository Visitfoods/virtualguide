...
test.describe('Configuração do Firebase - contactosinfoquestion', () => {
  test('deve conseguir enviar dados para o Firebase', async ({ page }) => {
    // Navegar para a página inicial
    await page.goto('http://localhost:3000');
    
    // Clicar no botão "FALAR COM O GUIA REAL" para abrir o popup
    await page.click('text=FALAR COM O GUIA REAL');
    ...
    await page.click('button:has-text("FALAR COM GUIA REAL")');
  ...
  test('deve mostrar erro se os campos estiverem vazios', async ({ page }) => {
    ...
    // Clicar no botão "FALAR COM O GUIA REAL"
    await page.click('text=FALAR COM O GUIA REAL');
