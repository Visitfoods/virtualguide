...
export const saveContactRequest = async (data: ContactFormData): Promise<string> => {
  try {
    ...
    // Adicionar documento à coleção 'contactosinfoquestion'
    const docRef = await addDoc(collection(db, 'contactosinfoquestion'), dataWithTimestamp);
...
export const clearConversationCookies = (conversationId: string): void => {
  try {
    ...
    if (storedConversationId === conversationId) {
      ...
      console.log('Cookies da conversa limpos com sucesso');
    }
  } catch (error) {
...
export const deleteAllConversations = async (): Promise<{ success: boolean; deletedCount: number; error?: string }> => {
  console.log('deleteAllConversations iniciada');
  ...
  try {
    ...
    console.log('Obtendo referência da coleção conversations...');
    const conversationsRef = collection(db, 'conversations');
    console.log('Obtendo snapshot das conversas...');
    const conversationsSnapshot = await getDocs(conversationsRef);
    
    console.log('Número de conversas encontradas:', conversationsSnapshot.size);
    
    if (conversationsSnapshot.empty) {
      console.log('Nenhuma conversa encontrada para apagar');
    ...
    conversationsSnapshot.docs.forEach((doc, index) => {
      console.log(`Conversa ${index + 1}:`, doc.id, doc.data());
    });
    
    // Apagar cada conversa uma por vez para evitar problemas de permissão
    console.log('Iniciando apagamento de conversas...');
    let deletedCount = 0;
    
    for (const doc of conversationsSnapshot.docs) {
      try {
        console.log(`Apagando conversa ${doc.id}...`);
        await deleteDoc(doc.ref);
        console.log(`Conversa ${doc.id} apagada com sucesso`);
        deletedCount++;
      } catch (error) {
    ...
    console.log(`Apagadas ${deletedCount} conversas com sucesso`);
